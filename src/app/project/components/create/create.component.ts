      ...
      .subscribe(response => {
        if (response.Success == true) {
          ...
          if (response.Data.Start_Date || response.Data.End_Date) {
            ...
            var startDateFormat=moment(response.Data.Start_Date).toDate();
            startDate = <NgbDateStruct>{
              year: startDateFormat.getFullYear(),
              month: startDateFormat.getMonth() + 1, day: startDateFormat.getDate()
            };

            var endDateFormat=moment(response.Data.End_Date).toDate();
            endDate = <NgbDateStruct>{
              year: endDateFormat.getFullYear(),
              month: endDateFormat.getMonth() + 1, day: endDateFormat.getDate()
            };
